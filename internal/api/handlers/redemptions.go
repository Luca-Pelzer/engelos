...
// channelFromRequest resolves the channel a handler operates on. When the route
// is mounted under /channels/{channelSlug} the WorkspaceMiddleware has already
// resolved and authorized a workspace, so its slug is the trusted channel and
// the client-supplied fallback is ignored. Otherwise (legacy ?channel= routes)
// the fallback value is used. This lets every handler serve both the new
// workspace-scoped path and the old path during the migration.
func channelFromRequest(r *http.Request, fallback string) string {
	if wc, ok := middleware.WorkspaceFromContext(r.Context()); ok {
		return wc.Workspace.Slug
	}
	return normChannel(fallback)
}
