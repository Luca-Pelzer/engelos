...
// maxPlayedPerChannel caps how many status=played rows are retained per
// (tenant, channel). After each successful Next promotion the store prunes
// older played rows beyond this many so the table does not grow unbounded.
const maxPlayedPerChannel = 200
...
// Next atomically advances the queue for a (tenant, channel): it demotes
// any rows still in status=playing to status=played, promotes the oldest
// status=queued row to status=playing, and prunes stale played rows. All
// three steps run inside one transaction under the store mutex, so there
// is never more than one playing row even if a prior Next left one behind.
// Returns ErrEmpty when nothing is queued (no demote or prune happens in
// that case, leaving any current playing row untouched).
func (s *sqliteStore) Next(ctx context.Context, tenantID, channel string) (Item, error) {
	...
	if err != nil {
		return Item{}, fmt.Errorf("songqueue: next begin: %w", err)
	}
	...
	const demote = `UPDATE song_queue SET status = ?
                    WHERE tenant_id = ? AND channel = ? AND status = ?`
	if _, err := tx.ExecContext(ctx, demote,
		StatusPlayed, tenantID, channel, StatusPlaying); err != nil {
		return Item{}, fmt.Errorf("songqueue: next demote: %w", err)
	}
	...
	if err := prunePlayed(ctx, tx, tenantID, channel); err != nil {
		return Item{}, err
	}

	if err := tx.Commit(); err != nil {
		return Item{}, fmt.Errorf("songqueue: next commit: %w", err)
	}
	it.Status = StatusPlaying
	return it, nil
}

// prunePlayed deletes status=played rows beyond the newest
// maxPlayedPerChannel for the (tenant, channel), keeping the table
// bounded. It runs inside the caller's transaction. The subquery picks
// the ids to keep (highest positions) and the outer DELETE removes the
// rest, so ordering of live rows is never disturbed.
func prunePlayed(ctx context.Context, tx *sql.Tx, tenantID, channel string) error {
	const prune = `
DELETE FROM song_queue
WHERE tenant_id = ? AND channel = ? AND status = ?
  AND id NOT IN (
    SELECT id FROM song_queue
    WHERE tenant_id = ? AND channel = ? AND status = ?
    ORDER BY position DESC
    LIMIT ?
  )`
	if _, err := tx.ExecContext(ctx, prune,
		tenantID, channel, StatusPlayed,
		tenantID, channel, StatusPlayed, maxPlayedPerChannel); err != nil {
		return fmt.Errorf("songqueue: next prune played: %w", err)
	}
	return nil
}
