...
func countByStatus(t *testing.T, s Store, tenant, channel, status string) int {
	t.Helper()
	ss, ok := s.(*sqliteStore)
	...
	err := ss.db.QueryRowContext(context.Background(),
		...
		tenant, normalizeChannel(channel), status).Scan(&n)
	require.NoError(t, err)
	return n
}

func TestNext_NeverLeavesTwoPlaying(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	...
	assert.Equal(t, 1, countByStatus(t, s, "local", "chan-A", StatusPlaying),
		"exactly one row must be playing after two Next calls")

	playedCount := countByStatus(t, s, "local", "chan-A", StatusPlayed)
	assert.Equal(t, 1, playedCount, "the prior playing row must be demoted to played")

	cur, err := s.Current(ctx, "local", "chan-A")
	require.NoError(t, err)
	assert.Equal(t, second.ID, cur.ID, "Current must resolve to the single playing row")
	assert.Equal(t, "vid2", cur.VideoID)
}

func TestNext_PrunesPlayedRowsToCap(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()

	total := maxPlayedPerChannel + 25
	for i := 0; i < total; i++ {
		_, err := s.Enqueue(ctx, sampleItem("local", "chan-A",
			"vid", "Song"))
		require.NoError(t, err)
	}
	...
	played := countByStatus(t, s, "local", "chan-A", StatusPlayed)
	assert.LessOrEqual(t, played, maxPlayedPerChannel,
		"played rows must be capped at maxPlayedPerChannel")
	assert.Equal(t, maxPlayedPerChannel, played,
		"after draining the queue the played rows settle at the cap")
	assert.Equal(t, 1, countByStatus(t, s, "local", "chan-A", StatusPlaying),
		"still exactly one playing row")
}
