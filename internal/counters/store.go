...
// maxInt64Str and minInt64Str are the int64 bounds as decimal literals,
// inlined into the Add upsert SQL so the saturating CASE guard clamps
// instead of overflowing the column. SQLite parses these as 64-bit
// integers; math.MaxInt64 / math.MinInt64 are duplicated as strings here
// because the value is spliced into a query string, not bound as a param.
const (
...
func (s *sqliteStore) Add(ctx context.Context, tenantID, channel, name string, delta int64) (Counter, error) {
	...
	//
	// The CASE guard saturates instead of overflowing: if adding a positive
	// delta would exceed maxInt64 (value > maxInt64 - delta) we clamp to
	// maxInt64; if adding a negative delta would drop below minInt64
	// (value < minInt64 - delta) we clamp to minInt64. Without this the
	// sum overflows int64, SQLite silently stores it as a float, and every
	// later int64 Scan of that row fails forever. The guard arithmetic
	// itself is overflow-safe because (maxInt64 - delta) and
	// (minInt64 - delta) never overflow for the delta sign each branch
	// tests.
	const up = `
...
DO UPDATE SET value =
	CASE
		WHEN excluded.value > 0 AND value > ` + maxInt64Str + ` - excluded.value THEN ` + maxInt64Str + `
		WHEN excluded.value < 0 AND value < ` + minInt64Str + ` - excluded.value THEN ` + minInt64Str + `
