...
func (s *Service) Evaluate(ctx context.Context, msg Message) Decision {
	...
	// A link violation can be waived once by a moderator-granted permit. In
	// dry-run we must not burn the one-shot permit, so only Consume when we are
	// actually enforcing; the preview simply treats the violation as standing.
	if result.FilterName == "links" && !dryRun && s.permits.Consume(msg.Channel, msg.Username) {
		return Decision{Kind: ActionNone}
	}
	...
	// filter still climbs the ladder to timeouts and eventually a ban. In
	// dry-run we Peek (non-mutating preview of the next offense) so shadow mode
	// never poisons the persistent ladder; only active mode Records.
	var action automodstate.Action
	var duration time.Duration
	if dryRun {
		action, duration = s.escal.Peek(msg.Channel, msg.Username, result.FilterName)
	} else {
...
func mergeVerdict(result automod.FilterResult, escAction automodstate.Action, escDur time.Duration) (ActionKind, time.Duration) {
	...
	escKind := escalActionKind(escAction)
