...
// maxRandomSpan caps the size of a $(random a b) interval. A viewer could
// otherwise request a span near the full int64 range; we refuse anything
// wider than a billion both to keep the result sensible and to stay well
// clear of the int overflow that computing the span would trigger.
const maxRandomSpan = 1_000_000_000
...
// operand, when a > b, or when the span overflows / is absurdly large.
//
// The span computation b-a+1 can overflow int64 for viewer-controlled
// operands (e.g. a=math.MinInt64, b=math.MaxInt64). A wrapped span is <= 0,
// and randIntN(n) panics for n <= 0, so we detect the overflow (b >= a but
// span <= 0) and bail before ever calling randIntN with a bad argument.
func randomRange(rest string, randIntN func(int) int) string {
	...
	span := b - a + 1
	// span <= 0 here means the subtraction wrapped around (overflow), since
	// b >= a was already established above. Refuse it.
	if span <= 0 {
		return ""
	}
	if span > maxRandomSpan {
		return ""
	}
	return strconv.Itoa(a + randIntN(span))
}
