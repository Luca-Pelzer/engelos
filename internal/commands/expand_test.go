...
// panicOnNonPositive is a randIntN stub that fails the test if it is ever
// called with n <= 0, the argument that makes the real rand.Intn panic. It
// proves randomRange's overflow guard refuses bad spans BEFORE calling the
// RNG.
func panicOnNonPositive(t *testing.T) func(int) int {
	t.Helper()
	return func(n int) int {
		if n <= 0 {
			t.Fatalf("randIntN called with n=%d (<= 0); overflow guard failed", n)
		}
		return 0
	}
}

func TestResolveVar_Random_Int64BoundsNoPanic(t *testing.T) {
	deps := expandDeps{now: fixedClock(), randIntN: panicOnNonPositive(t)}
	// The full int64 interval: b-a+1 overflows to <= 0 and would panic the
	// real rand.Intn. The guard must turn this into "" with no RNG call.
	got := expandWith("$(random -9223372036854775808 9223372036854775807)", sampleMsg(), nil, deps)
	assert.Equal(t, "", got)
}

func TestRandomRange_OverflowAndCaps(t *testing.T) {
	rng := panicOnNonPositive(t)
	// Full-range and other overflowing/oversized spans all yield "".
	assert.Equal(t, "", randomRange("-9223372036854775808 9223372036854775807", rng))
	assert.Equal(t, "", randomRange("0 9223372036854775807", rng))
	// Span just over the one-billion cap is refused.
	assert.Equal(t, "", randomRange("0 1000000000", rng))
	// A sane in-cap span still works (zero stub picks the low end).
	assert.Equal(t, "0", randomRange("0 999999999", rng))
	assert.Equal(t, "-5", randomRange("-5 5", rng))
}
