...
func TestResolver_DynamicRandomFullRangeCannotPanicHandle(t *testing.T) {
	// A custom command whose response is the full int64 $(random ...) range
	// is the C1 crash vector: with the real RNG it would overflow and panic
	// out of Handle, killing the dispatcher goroutine. Handle must absorb it
	// and return a non-panicking, empty-on-bad-expansion reply.
	r := &fakeResolver{
		rc:  commands.ResolvedCommand{Response: "$(random -9223372036854775808 9223372036854775807)"},
		hit: true,
	}
	e := commands.New(commands.Config{Logger: silentLogger(), Resolver: r})

	require.NotPanics(t, func() {
		reply, ok := e.Handle(context.Background(), msgText("!roll"))
		assert.True(t, ok)
