...
func (e *Escalator) Record(channel, user, filter string) (Action, time.Duration) {
	...
	return e.tierForCount(rec.count)
}

// tierForCount maps a 1-based offense count onto the tier ladder, returning the
// Action and (for timeouts) duration for that rung. A count past the final rung
// stays pinned to the last (most severe) tier so a persistent offender keeps
// getting the maximum punishment. Record and Peek both route through this so
// their ladder mapping can never drift apart.
func (e *Escalator) tierForCount(count int) (Action, time.Duration) {
	idx := count - 1
	if idx >= len(e.tiers) {
		idx = len(e.tiers) - 1
	}
...
// Peek returns the Action and (for timeouts) duration that Record WOULD return
// for the next offense on (channel, user, filter), WITHOUT mutating any state.
// It applies the same decay rule as Record (a stale record reads as zero) and
// maps the resulting next count (current offenses + 1) onto the same ladder.
// Dry-run / shadow mode uses this to preview the escalating punishment without
// poisoning the persistent offense counter.
func (e *Escalator) Peek(channel, user, filter string) (Action, time.Duration) {
	e.mu.Lock()
	defer e.mu.Unlock()

	current := 0
	rec := e.records[escalKey(channel, user, filter)]
	if rec != nil && rec.count > 0 && e.now().Sub(rec.lastOffense) <= e.decay {
		current = rec.count
	}
	return e.tierForCount(current + 1)
}
