...
func TestTick_GraceWindow_ConsecutiveInGraceAdvances(t *testing.T) {
	t.Parallel()
	cfg := streak.DefaultConfig() // 6h grace
	s, clk := newSystem(t, cfg)
	ctx := context.Background()

	clk.set(time.Date(2026, 5, 29, 2, 0, 0, 0, time.UTC))
	r1, err := s.Tick(ctx, testTenant, testChannel, testViewer, testUser)
	require.NoError(t, err)
	require.Equal(t, 1, r1.DaysCurrent)

	clk.set(time.Date(2026, 5, 30, 2, 0, 0, 0, time.UTC))
	r2, err := s.Tick(ctx, testTenant, testChannel, testViewer, testUser)
	require.NoError(t, err)
	assert.False(t, r2.SameDayReTick, "02:00 next day is a distinct grace-shifted day")
	assert.Equal(t, 2, r2.DaysCurrent, "consecutive ticks inside the grace window must advance")
}
