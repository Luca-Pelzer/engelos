...
func (rm *ReadModel) applyStarted(e eventsourcing.Event) error {
	...
	s.LastTickDayUTC = tickDayOrFallback(p.TickDayUTC, e.OccurredAt)
...
// tickDayOrFallback returns the grace-adjusted day stamped by the write path
// (System.Tick via streakDay(now, GraceWindow)). Events predating that field,
// and manual UseFreeze events, carry a zero TickDayUTC and fall back to the
// no-grace day of occurredAt to preserve historical behaviour.
func tickDayOrFallback(tickDay, occurredAt time.Time) time.Time {
	if !tickDay.IsZero() {
		return tickDay
	}
	return streakDay(occurredAt, 0)
}
