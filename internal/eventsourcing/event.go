...
// eventIDMu guards eventIDEntropy: ulid.Monotonic is stateful and not
// goroutine-safe, so concurrent NewEvent callers must serialise id minting.
// Monotonic entropy keeps same-millisecond ids sorted in append order, which
// the store's id-ordered replay and AfterID cursor depend on.
var (
...
func NewEvent(tenantID, eventType string, payload json.RawMessage) (Event, error) {
	...
	now := time.Now().UTC()
	eventIDMu.Lock()
	id, err := ulid.New(ulid.Timestamp(now), eventIDEntropy)
	eventIDMu.Unlock()
	if err != nil {
