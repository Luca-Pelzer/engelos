...
func NewEngine(cfg Config) (*Engine, error) {
	...
	for i, entry := range cfg.BannedWords.Entries {
		...
		// A regex that matches the empty string (e.g. "a*", ".*", "x?") would
		// fire on every message and mass-punish the whole chat, so reject it at
		// load time rather than let the footgun rule run.
		if m.re != nil && m.re.MatchString("") {
			return nil, fmt.Errorf("automod: banned entry %d (%q): regex matches the empty string and would fire on every message", i, entry.Phrase)
		}
