...
//   - /overlay/now-playing  - current song card.
//   - /overlay/song-player  - audio player surface for song requests.
//   - /overlay/wrapped      - year-in-review recap.
//   - /overlay/moment       - operator-triggered highlight card.
//
// The events and alerts surfaces both react to channel.points.redemption
// envelopes, so a redemption appears on stream the moment it is claimed.
//
// /overlay/ (empty name) serves a tiny index page listing the URLs so
