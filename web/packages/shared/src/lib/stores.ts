...
const ACTIVE_WORKSPACE_KEY = 'engelos.workspace';

function initialActiveWorkspace(): string {
  if (typeof localStorage === 'undefined') return '';
  try { return localStorage.getItem(ACTIVE_WORKSPACE_KEY) ?? ''; } catch { return ''; }
}

export const activeWorkspace = writable<string>(initialActiveWorkspace());

export function setActiveWorkspace(slug: string): void {
  activeWorkspace.set(slug);
  try { localStorage.setItem(ACTIVE_WORKSPACE_KEY, slug); } catch { /* ignore */ }
}
