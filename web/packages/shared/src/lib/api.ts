...
export function channelApi(slug: string) {
  const base = `/api/v1/channels/${encodeURIComponent(slug)}`;
  const at = (sub: string) => `${base}${sub}`;
  return {
    get:    <T = unknown>(sub: string, opts?: Omit<RequestOptions, 'method' | 'body'>) => request<T>(at(sub), { ...opts, method: 'GET' }),
    post:   <T = unknown>(sub: string, body?: unknown, opts?: Omit<RequestOptions, 'method' | 'body'>) => request<T>(at(sub), { ...opts, method: 'POST', body }),
    put:    <T = unknown>(sub: string, body?: unknown, opts?: Omit<RequestOptions, 'method' | 'body'>) => request<T>(at(sub), { ...opts, method: 'PUT', body }),
    delete: <T = unknown>(sub: string, opts?: Omit<RequestOptions, 'method' | 'body'>) => request<T>(at(sub), { ...opts, method: 'DELETE' }),
  };
}
