<script lang="ts">
  ...
  import { channelApi, toast, activeWorkspace } from '@engelos/shared/lib';
  ...
  async function load() {
    ...
    loading = true;
    try {
      const c = await channelApi(channel).get<Config>('/translate');
      enabled = c.enabled;
  ...
  async function save() {
    ...
    saving = true;
    try {
      await channelApi(channel).put<Config>('/translate', {
        enabled,
...
<section class="space-y-6">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um die Uebersetzung zu konfigurieren.</p>
    </Card>
  {/if}
