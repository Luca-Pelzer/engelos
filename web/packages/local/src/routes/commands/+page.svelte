<script lang="ts">
  ...
  async function load() {
    ...
    loading = true;
    try {
      const res = await channelApi(channel).get<ListResponse>('/commands');
  ...
  async function save(e: SubmitEvent) {
    ...
    try {
      if (editingName) {
        await channelApi(channel).put(`/commands/${encodeURIComponent(editingName)}`, {
          response,
          min_role: fMinRole,
        });
        toast('Command updated.', 'success');
      } else {
        await channelApi(channel).post('/commands', {
          name,
          response,
          min_role: fMinRole,
        });
  ...
  async function remove(c: Command) {
    ...
    try {
      await channelApi(channel).delete(`/commands/${encodeURIComponent(c.name)}`);
...
<section class="space-y-6">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um Commands zu verwalten.</p>
    </Card>
  {/if}
