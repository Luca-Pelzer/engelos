<script lang="ts">
  ...
  async function load() {
    ...
    loading = true;
    try {
      const c = await channelApi(channel).get<Config>('/cohost');
      enabled = c.enabled;
      botName = c.bot_name;
      persona = c.persona;
  ...
  async function save() {
    ...
    saving = true;
    try {
      await channelApi(channel).put<Config>('/cohost', {
        enabled,
        bot_name: botName,
        persona,
...
<section class="space-y-6">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um den Co-Host zu konfigurieren.</p>
    </Card>
  {/if}
