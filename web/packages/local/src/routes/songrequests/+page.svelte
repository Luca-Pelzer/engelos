<script lang="ts">
  ...
  async function load() {
    ...
    loading = true;
    try {
      const c = await channelApi(channel).get<Config>('/songrequests');
      provider = c.provider;
  ...
  async function save() {
    ...
    saving = true;
    try {
      await channelApi(channel).put<Config>('/songrequests', {
        provider,
...
<section class="space-y-6">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um Song-Requests zu konfigurieren.</p>
    </Card>
  {/if}
