<script lang="ts">
  ...
  async function load() {
    ...
    loading = true;
    try {
      const res = await channelApi(channel).get<ListResponse>('/counters');
  ...
  async function create(e: SubmitEvent) {
    ...
    saving = true;
    try {
      await channelApi(channel).put(`/counters/${encodeURIComponent(name)}`, {
        value,
      });
  ...
  async function bump(c: Counter, delta: number) {
    ...
    try {
      const updated = await channelApi(channel).post<Counter>(
        `/counters/${encodeURIComponent(c.name)}/add`,
        { delta },
      );
  ...
  async function setValue(c: Counter) {
    ...
    busy = { ...busy, [c.name]: true };
    try {
      const updated = await channelApi(channel).put<Counter>(`/counters/${encodeURIComponent(c.name)}`, {
        value,
      });
      counters = counters.map((x) => (x.name === c.name ? updated : x));
  ...
  async function remove(c: Counter) {
    ...
    try {
      await channelApi(channel).delete(`/counters/${encodeURIComponent(c.name)}`);
...
<section class="space-y-6">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um Counter zu verwalten.</p>
    </Card>
  {/if}
