<script lang="ts">
  ...
  async function load() {
    ...
    loading = true;
    try {
      const c = await channelApi(channel).get<Config>('/clipper');
      enabled = c.enabled;
  ...
  async function save() {
    ...
    saving = true;
    try {
      await channelApi(channel).put<Config>('/clipper', {
        enabled,
...
<section class="space-y-6 max-w-3xl">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um den Auto-Clipper zu konfigurieren.</p>
    </Card>
  {/if}
