<script lang="ts">
  ...
  async function load() {
    ...
    loading = true;
    try {
      const res = await channelApi(channel).get<ListResponse>('/redemptions');
  ...
  async function save(e: SubmitEvent) {
    ...
    try {
      if (editingRewardID) {
        await channelApi(channel).put(`/redemptions/${encodeURIComponent(editingRewardID)}`, {
        ...
        await channelApi(channel).post('/redemptions', {
          reward_id: rewardID,
          reward_title: fRewardTitle.trim(),
          action_type: fActionType,
          action_param: paramRequired ? param : '',
  ...
  async function toggleEnabled(b: Binding) {
    ...
    try {
      await channelApi(channel).post(`/redemptions/${encodeURIComponent(b.reward_id)}/enabled`, {
        enabled: next,
      });
  ...
  async function remove(b: Binding) {
    ...
    try {
      await channelApi(channel).delete(`/redemptions/${encodeURIComponent(b.reward_id)}`);
...
<section class="space-y-6">
  ...
  {#if !channel}
    <Card class="reveal-up reveal-up-delay-1">
      <p class="text-[13px] text-fg-soft">Waehle oben einen Workspace, um Channel-Points zu verwalten.</p>
    </Card>
  {/if}
      ...
      <form onsubmit={save} class="p-6 space-y-4">
        <header class="mb-1">
          ...
          <p class="text-[12.5px] text-fg-soft mt-0.5">
            Channel <span class="font-mono text-accent">{channel}</span>
          </p>
        </header>
        ...
        <Input
          ...
          placeholder="Optional, nur zur Anzeige"
