<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      toast(err instanceof ApiException && err.status === 501 ? 'The timers feature is not enabled.' : 'Could not load.', 'error');
    } finally {
      loading = false;
    }
  }
  ...
  async function save() {
    if (!channel || !fName.trim() || !fMessage.trim()) { toast('Name and message are required.', 'warn'); return; }
...
<section class="page" data-screen-label="timers">
  <div class="page-wrap">
    <div class="section-title">Schedule Triggers <span class="sub">timer-based workflow starts, not a standalone bot feature</span></div>
    <div class="toolbar">
      ...
      <div class="grow"></div>
      <span class="count-pill"><b>{timers.length}</b> Schedule Triggers</span>
      <button class="btn btn-primary btn-sm" onclick={openNew} disabled={!channel}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14" /></svg>Neuer Schedule Trigger</button>
    </div>

    {#if showForm}
      <div class="form-card">
        <div class="form-grid">
          <div><label class="fld" for="timer-name">Name</label><div class="input"><input id="timer-name" type="text" placeholder="e.g. discord" bind:value={fName} disabled={editing !== null} /></div></div>
        ...
        <div><label class="fld" for="timer-message">Message</label><div class="input"><input id="timer-message" type="text" placeholder="Join our Discord: ..." bind:value={fMessage} /></div></div>
        <div class="form-grid">
    ...
    <table class="dtable" style="margin-top:14px">
      <thead><tr><th>Name</th><th>Message</th><th class="right" style="width:110px">Interval</th><th style="width:90px">Active</th><th class="right" style="width:96px">Action</th></tr></thead>
      <tbody>
        {#each timers as t (t.name)}
          <tr>
            ...
            <td><button class="switch" class:on={t.enabled} onclick={() => toggle(t)} aria-label="Toggle"></button></td>
            <td>
              <div class="row-actions">
                <button class="iact" onclick={() => openEdit(t)} aria-label="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z" /></svg></button>
                <button class="iact del" onclick={() => del(t.name)} aria-label="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h16M9 7V5h6v2M6 7l1 13h10l1-13" /></svg></button>
              </div>
            </td>
          </tr>
        {/each}
    ...
    {#if timers.length === 0}
      <div class="empty">
        ...
        <div class="t">{channel ? 'No schedule triggers' : 'Select a workspace above'}</div>
